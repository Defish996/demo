...
# 汇总表名
TOTAL_TABLE = 'delay_total'

# 任务配置里去掉 table_name，只保留名字和目录
TASKS = [
...
def create_total_table(conn: pymysql.Connection):
    """只建一次汇总表"""
    sql = f"""
    CREATE TABLE IF NOT EXISTS {TOTAL_TABLE} (
      ...
      PRIMARY KEY (ID, Calc_Date, ts, Exchange, category)
...
def process_csv_file(src: Path, c_date: str, category: str, conn: pymysql.Connection) -> int:
    """处理单个 CSV 文件并写入汇总表"""
    ...
    with src.open(newline='', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            for minute, delay_str in zip(minutes, delays):
                if delay_str.strip() != '':          # 不再排除 '0'
                    try:
                        delay = float(delay_str)
                        buffer.append((_id, minute, delay, exchange, c_date, category))
                    except ValueError:
                        continue
...
def run_task(task: Dict[str, Any], c_date: str, conn: pymysql.Connection):
    ...
    print(f"\n--- 开始处理任务: {task['name']} ---")
    in_dir = task['in_dir']
    category = CATEGORY_MAP[task['name']]  # 取类别
    ...
    # 建表只在第一次任务时做
    create_total_table(conn)

    total_files = 0
    total_rows = 0
    for src in csv_files:
        try:
            rows = process_csv_file(src, c_date, category, conn)
            total_files += 1
            total_rows += rows
