...
表名：trd_sys_order_first_ack_min
"""
import re
import csv
...
# ---------- 2. 目录 & 交易所映射 ----------
IN_DIR      = Path('3/min/交易系统下单首次回报延迟')          # CSV 所在目录
...
def process_one(src: Path, c_date: str, conn):
    ...
    sql = """
        INSERT INTO trd_sys_order_first_ack_min
...
def main():
    ...
    try:
        # 自动建表（已存在则跳过）
        create_sql = """
        CREATE TABLE IF NOT EXISTS trd_sys_order_first_ack_min (
